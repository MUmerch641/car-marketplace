      ...
      <section className="min-w-0 md:ml-64 md:h-screen md:overflow-y-auto">
        <div className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-slate-200 bg-white/95 px-5 backdrop-blur sm:px-8">
          <div>
            ...
            <p className="text-sm font-bold text-[#082a30]">Operations centre</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/" className="text-xs font-semibold text-slate-600 hover:text-[#e94a3f] sm:text-sm">View Shaz website ↗</Link>
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#082a30] text-sm font-bold text-white">{(profile.full_name || "A").slice(0, 1).toUpperCase()}</div>
          </div>
        </div>
        {children}
      </section>

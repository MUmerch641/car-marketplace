...
export default async function Page() {
  ...
  return (
    <main className="p-5 lg:p-8">
      ...
      {/* Metric cards — each CountUp triggers individually when in view */}
      <AnimatedContent distance={14} duration={0.5} delay={0.1} className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map(([l, n]) => (
          <div key={String(l)} className="border bg-white p-5">
            <p className="text-sm text-[#667085]">{l}</p>
            <CountUp
              to={Number(n ?? 0)}
              duration={0.7}
      ...
      <AnimatedContent distance={14} duration={0.5} delay={0.05} className="mt-8">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <Link
            href="/admin/cars"
            className="border bg-white p-4 transition-colors hover:border-brand"
          >
            {cars ?? 0} cars awaiting moderation
          </Link>
          <Link
            href="/admin/bookings?status=pending"
            className="border bg-white p-4 transition-colors hover:border-brand"
          >
            {bookings ?? 0} bookings awaiting confirmation
          </Link>
          <Link
            href="/admin/verifications"
            className="border bg-white p-4 transition-colors hover:border-brand"
          >
            {verification ?? 0} verification actions
          </Link>
        </div>
      </AnimatedContent>

      {/* Recent activity — no row-by-row stagger */}
      <AnimatedContent distance={14} duration={0.5} delay={0.1} className="mt-8">
        ...
        {audit?.length ? (
          <div className="mt-3 space-y-2">
            {audit.map((x, i) => (
              <div className="border bg-white p-3 text-sm" key={i}>
                {x.action.replaceAll("_", " ")} · {x.entity_type} ·{" "}
                {new Date(x.created_at).toLocaleString("en-GB")}
              </div>
            ))}
          </div>
        ) : (
          <p className="mt-3 border border-dashed p-5 text-[#667085]">

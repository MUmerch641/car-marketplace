    ...
    <main className="bg-[#f7f8fa] py-7 sm:py-10">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <header className="border-b border-slate-200 pb-5">
          <p className="text-xs font-bold uppercase tracking-[.12em] text-[#e94a3f]">Sell your car</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-[#082a30]">{car.year} {vehicleName}</h1>
        ...
        {car.rejection_reason && <p className="mt-5 rounded-xl border-l-4 border-[#e94a3f] bg-red-50 p-4 text-sm text-red-800">Review feedback: {car.rejection_reason}</p>}

        <section className="mt-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          {step === "details" && <><div className="mb-6 flex items-start justify-between gap-4 border-b border-slate-200 pb-5"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#e94a3f]">Step 1 of 3</p><h2 className="mt-1 text-2xl font-bold text-[#082a30]">Vehicle details</h2><p className="mt-1 text-sm text-slate-600">Update the information buyers will see.</p></div><Link href={`/dashboard/cars/${car.id}/edit?step=photos`} className="text-sm font-semibold text-[#e94a3f]">Back to photos</Link></div><ListingForm action={action} car={car as unknown as Record<string, string |number | boolean | null>} isActive={isActive} /></>}

          {step === "photos" && <><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><Link href={`/dashboard/cars/${car.id}/edit?step=details`} className="inline-flex items-center gap-1 text-sm font-semibold text-slate-700 hover:text-[#082a30]"><Pencil size={15} /> Edit vehicle details</Link>{images.length > 0 && <Link href={`/dashboard/cars/${car.id}/edit?step=review`} className="inline-flex items-center rounded-lg bg-[#082a30] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#173553]">Review listing <ChevronRight size={16} className="ml-1" /></Link>}</div><ImageManager carId={car.id} userId={user.id} images={images} />{images.length > 0 && <div className="mt-6 flex justify-end border-t border-slate-200 pt-5"><Link href={`/dashboard/cars/${car.id}/edit?step=review`} className="inline-flex items-center rounded-lg bg-[#082a30] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#173553]">Review listing <ChevronRight size={16} className="ml-1" /></Link></div>}</>}

          {step === "review" && <div><div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-5"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#e94a3f]">Step 3 of 3</p><h2 className="mt-1 text-2xl font-bold text-[#082a30]">Review your listing</h2><p className="mt-1 text-sm text-slate-600">Check the important details before submitting to Shaz.</p></div><Link href={`/dashboard/cars/${car.id}/edit?step=photos`} className="inline-flex items-center gap-1 text-sm font-semibold text-[#e94a3f]"><Upload size={15} /> Edit photos</Link></div>{images.length ? <><div className="mt-5 grid gap-5 sm:grid-cols-[180px_1fr]"><div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-100">{images[0].url && <ImageManagerPreview url={images[0].url} />}</div><dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm"><div><dt className="text-slate-500">Vehicle</dt><dd className="mt-1 font-semibold text-[#082a30]">{car.year} {vehicleName}{car.variant ? ` ${car.variant}` : ""}</dd></div><div><dt className="text-slate-500">Price</dt><dd className="mt-1 font-semibold text-[#082a30]">£{Number(car.price).toLocaleString("en-GB")}</dd></div><div><dt className="text-slate-500">Mileage</dt><dd className="mt-1 font-semibold text-[#082a30]">{Number(car.mileage).toLocaleString("en-GB")} miles</dd></div><div><dt className="text-slate-500">Photos</dt><dd className="mt-1 font-semibold text-[#082a30]">{images.length} uploaded</dd></div><div><dt className="text-slate-500">Location</dt><dd className="mt-1 font-semibold text-[#082a30]">{car.city}, {car.postcode}</dd></div><div><dt className="text-slate-500">Status</dt><dd className="mt-1 font-semibold text-[#082a30] capitalize">{car.status}</dd></div></dl></div><div className="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between"><Link href={`/dashboard/cars/${car.id}/edit?step=details`} className="text-sm font-semibold text-slate-700">Edit vehicle details</Link>{!isActive && <form action={async () => { "use server"; await submitListingAction(car.id); }}><button type="submit" className="rounded-lg bg-[#e94a3f] px-5 py-3 text-sm font-semibold text-white hover:bg-[#c73830]">Submit for review</button></form>}</div></> : <div className="py-8"><p className="font-semibold text-[#082a30]">Add at least one photo before submitting.</p><Link href={`/dashboard/cars/${car.id}/edit?step=photos`} className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-[#e94a3f]">Add photos <ChevronRight size={15} /></Link></div>}</div>}
        </section>
      </div>
    </main>
  );

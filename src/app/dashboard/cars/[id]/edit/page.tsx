...
import { submitListingAction, updateListingAction } from "@/app/marketplace-actions";
...
export default async function EditCarPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ step?: string; created?: string }> }) {
  ...
  if (!car) notFound();
  if (!['draft', 'rejected'].includes(car.status)) return <main className="mx-auto max-w-3xl px-5 py-12"><h1 className="text-2xl font-bold">This listing cannot be edited right now.</h1><p className="mt-2 text-[#667085]">Pending and active listings are locked while they are being reviewed or shown to buyers.</p></main>;
    ...
    <main className="bg-[#f7f8fa] py-7 sm:py-10">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <header className="border-b border-slate-200 pb-5">
          ...
          <p className="mt-2 text-sm text-slate-600">Complete your listing, then send it to Fengxing for review.</p>
        </header>

...
export async function assignWorkerAction(bookingId: string, form: FormData) { await requireRole("admin"); const workerId = field(form, "workerId"); if (!workerId) return { error: "Choose an authorised field worker." }; const supabase = await createClient(); const { error } = await supabase.rpc("assign_service_worker", { p_booking_id: bookingId, p_worker_id: workerId }); if (error) return { error: "Unable to assign this worker." }; revalidatePath("/admin/bookings"); revalidatePath("/inspector"); return { success: "Worker assigned." }; }

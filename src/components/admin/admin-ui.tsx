...
export function AdminPageHeader({ eyebrow = "Operations", title, description, action }: { eyebrow?: string; title: string; description?: string; action?: ReactNode }) {
  return <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#e94a3f]">{eyebrow}</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[#082a30] sm:text-4xl">{title}</h1>{description && <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{description}</p>}</div>{action && <div className="shrink-0">{action}</div>}</header>;
}
...
export const adminInput = "w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#e94a3f] focus:ring-4 focus:ring-red-100";
export const adminButton = "inline-flex items-center justify-center rounded-lg bg-[#e94a3f] px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#c73830] focus:outline-none focus:ring-4 focus:ring-red-100";

...
export function VerificationForm({ carId, inspectionType = "buyer_inspection" }: { carId?: string; inspectionType?: "seller_pre_inspection" | "buyer_inspection" }) {
  ...
  return <form action={action} className="grid gap-4 md:grid-cols-2">
    ...
    <button disabled={pending} className="md:col-span-2 rounded-lg bg-[#d92d20] px-5 py-3 text-sm font-semibold text-white hover:bg-[#b42318] disabled:cursor-not-allowed disabled:opacity-60">{pending ? "Requesting inspection…" : inspectionType === "seller_pre_inspection" ? "Request Fengxing Inspection" : "Request inspection"}</button>
  </form>;
}

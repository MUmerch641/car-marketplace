  ...
  return (
    <section aria-labelledby="listing-photos-heading">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.12em] text-[#e94a3f]">Step 2 of 3</p>
          <h2 id="listing-photos-heading" className="mt-1 text-2xl font-bold text-[#082a30]">Add photos</h2>
        ...
        <label className="inline-flex w-fit cursor-pointer items-center rounded-lg bg-[#e94a3f] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#c73830]">
      ...
      {images.length ? (
          ...
          {images.map((image, index) => (
            <article key={image.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              ...
              <div className="flex items-center justify-between gap-2 p-3 text-xs font-semibold">
                {image.is_primary ? <span className="inline-flex items-center gap-1 text-emerald-700"><Star size={14} fill="currentColor" /> Primary photo</span> : <button disabled={pending} onClick={() => makePrimary(image)} className="text-[#e94a3f] hover:text-[#c73830]">Make primary</button>}
        ...
        <div className="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-5 py-8 text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-[#082a30]">No photos uploaded yet</p>
